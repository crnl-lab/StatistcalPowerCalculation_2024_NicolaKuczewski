...
def show_widget():
    ...
    test_direction=W.Dropdown(value='Unilateral', options=['Unilateral','Bilateral'],ensure_option=True, description="Test direction",style={'description_width': 'initial'},layout=W.Layout(width='400px'))
    num_trial = W.IntText(value=4, description="Number of trials",style={'description_width': 'initial'},layout=W.Layout(width='400px'))
    chance_level= W.FloatText(value=50., description="Chance level (%)",style={'description_width': 'initial'},layout=W.Layout(width='400px'))
    ...
    hbox = W.VBox(children=[mode, method_name,test_direction, num_trial,chance_level,
                            success_rate_grp1, std_grp1, sample_size_grp1,
                            success_rate_grp2,std_grp2, sample_size_grp2,alpha_risk,num_iteration,num_simulation,
                            button, out])
    ...
    def on_mode_changed(change=None):
        ...
        if mode.value == 'one-group':
            chance_level.disabled = False
            success_rate_grp2.disabled = True
            std_grp2.disabled = True
            sample_size_grp2.disabled = True
            method_name.options = ['Ttest1','Proportion1','Generalized_estimating_equations1','Multilevel_Mixed_Model1']
            ...
            method_name.options = ['Ttest2','Proportion2','Welch','Generalized_estimating_equations','Multilevel_Mixed_Model']
    ...
    def on_button_run_clicked(change=None):
        ...
        run_simulation(
            ...
            chance_level=chance_level.value,
